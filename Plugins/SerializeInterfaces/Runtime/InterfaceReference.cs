	...
	public class InterfaceReference<TInterface, UObject> where UObject : Object where TInterface : class
	{
		...
		public TInterface Value
		{
			get
			{
				...
				if (@interface == null)
				{
					Debug.unityLogger.Log(LogType.Assert, $"{_underlyingValue} needs to implement interface {nameof(TInterface)}.");
				}
				return @interface;
			}
			set
			{
				...
				else
				{
					...
					if (newValue == null)
					{
						Debug.unityLogger.Log(LogType.Assert, $"{value} needs to be of type {typeof(UObject)}.");
					}
