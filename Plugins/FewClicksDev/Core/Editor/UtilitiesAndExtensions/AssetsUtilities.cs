namespace FewClicksDev.Core
{
    ...
    public static class AssetsUtilities
    {
        ...
        /// </summary>
        public static T GetAssetOfType<T>(string _name, string _folder = "") where T : Object
        {
            string _type = $"{typeof(T)}";
            _type = FixObjectTypeForFilter(_type);

            string _searchFilter = $"t:{_type} {_name}";
            string[] _guids = _folder.IsNullEmptyOrWhitespace()
                ? AssetDatabase.FindAssets(_searchFilter)
                : AssetDatabase.FindAssets(_searchFilter, new string[] { ConvertAbsolutePathToDataPath(_folder) });
        ...
        public static string GetAssetPath(this Object _object)
        {
            ...
            if (AssetDatabase.Contains(_object) == false)
            {
                return GetHierarchyPath(_object);
            }
        ...
        public static string GetHierarchyPath(Object _object)
        {
            if (_object == null)
            {
                return string.Empty;
            ...
            if (_object is not GameObject && _object is not Component)
            {
                return string.Empty;
            }
            ...
            Transform _startingTransform = _object switch
            {
                GameObject _gameObject => _gameObject.transform,
                Component _component => _component.transform,
                _ => null
            };

            if (_startingTransform == null)
            {
                return string.Empty;
            }

            string _path = _startingTransform.name;

            while (_startingTransform.parent != null)
            {
                _startingTransform = _startingTransform.parent;
                _path = _startingTransform.name + "/" + _path;
            }
